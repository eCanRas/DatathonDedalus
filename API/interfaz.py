...
async def main(message: cl.Message):
    ...
    procesando_msg = cl.Message(content="🛑 **Procesando...**", author="Asistente")
    await procesando_msg.send()

    try:
        async with httpx.AsyncClient() as client:
            # Enviar la petición a la API Flask
            response = await client.post(API_URL, json={"message": message.content}, timeout=20)
            response.raise_for_status()  # Lanza un error si la respuesta no es 200 OK

            # Convertir la respuesta a JSON
            data = response.json()
            respuesta_asistente = data.get("message", "⚠️ Error en la respuesta de la API")

    except httpx.HTTPStatusError as http_err:
        respuesta_asistente = f"❌ Error HTTP {http_err.response.status_code}: {http_err}"
    except httpx.RequestError as req_err:
        respuesta_asistente = f"⚠️ Error de conexión: {req_err}"
    except Exception as e:
        respuesta_asistente = f"⚠️ Error inesperado: {e}"
